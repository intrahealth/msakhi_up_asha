...
//	String msg=(String) spnsel.getSelectedItem();
	...
	try {       
		...
		Cursor server_id_data =mydb.getServer_id(slno);
	    int server_id = 0;
        ...
        payload.put("imdt", pdte);
        payload.put("im_id", sel);
	} catch (Exception e) {
...
//		if (rbBoy.isChecked()) sex="B";else sex="G";
	...
	Context ctx=getApplicationContext();
	  String phoneNumber=getString(R.string.smsno);
		...
		if (DBAdapter.send_sms)
		{
			mydb.sendGPRS(AppVariable.API_INDEX(mContext)+"immn", payload.toString(),1);
		}
		finish();
  }});
