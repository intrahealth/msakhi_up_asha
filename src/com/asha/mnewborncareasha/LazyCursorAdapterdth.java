...
//		dtPicker.init(Integer.parseInt(dt_str[0]),Integer.parseInt(dt_str[1])-1, Integer.parseInt(dt_str[2]), new OnDateChangedListener() {
        ...
        if (c.isNull(mdthCol) && c.isNull(cdthCol)) 
        	...
        	if (c.getInt(mstCol)==1) {
        		tvDob.setText("गर्भपात : "+DBAdapter.strtodate(c.getString(dobCol))+
        				"("+c.getInt(mnthCol)+")");
        		tvEdd.setText("");
        		} else {
        ...
        if (!c.isNull(mdthCol))
        	tvEdd.setText("मृत माँ : "+DBAdapter.strtodate(c.getString(mdthCol)));
        if (!c.isNull(cdthCol)) 
        	{ 
        	tvDob.setText("मृत शिशु : "+DBAdapter.strtodate(c.getString(cdthCol))+
    				"("+c.getString(mnthCol)+")");
        ...
        if (c.isNull(mdthCol) && c.isNull(cdthCol)) 
        	...
        	if (c.getInt(mstCol)==1) {
        		...
        		tvEdd.setText("");
            		...
            		tvDob.setText("मृत शिशु : "+DBAdapter.strtodate(c.getString(dobCol))+
            				"("+c.getInt(mnthCol)+")");
            		tvEdd.setText("");        			
        		}
        	}
