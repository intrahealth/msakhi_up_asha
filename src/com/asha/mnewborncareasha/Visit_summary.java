...
public class Visit_summary extends Activity {
	...
	// String
	// grpinfo[]={"Newborn check-up","Mother's check-up","Bacterial infection check-up","Diarrhea check-up","BreastFeeding check-up",""};

	// String
	// vst_seq[]={"¼1½ igyh tkap","¼2½ nwljh tkap","¼3½ rhljh tkap","¼4½ pkSFkh tkap","¼5½ ikapoh tkap","¼6½ Ìëh tkap","¼7½ lkroh tkap"};
	// String
	// vst_seq[]={"1. पहली जांच","2. दूसरी जांच","3. तीसरी जांच","4. चौथी जांच","5. पांचवी जांच","6. छठी जांच","7. सातवीं जांच"};
	// String
	...
	String grpinfo[] = { "शिशु की जांच", "माँ की जांच", "जीवाणु संक्रमण जांच",
			"दस्त रोग की जांच", "स्तनपान सम्बंधित जांच", "", "", "" };
	...
	private void addListenerOnButton() {
		...
		if (bflag) {
			...
			c = mydb.getVisitSummary(pid, false);
		} else {
			tvHeadVisit.setText("इस गृहभ्रमण के दौरान");
			btnBack.setVisibility(android.view.View.INVISIBLE);
			c = mydb.getVisitSummary(pid, fvisit);
		}
		...
		if (c.moveToFirst()) {
			tvPname.setText("गृहभ्रमण की तारीख :"
					+ c.getString(c.getColumnIndex("avd")));
			StringBuilder gsumm = new StringBuilder(c.getString(c
					.getColumnIndex("gsumm")));
			for (int i = 0; i < gsumm.length(); i++) {
				...
				if (!grpinfo[i].trim().equals("")) {
					...
					llp.setMargins(5, 5, 0, 5); // llp.setMargins(left, top,
												// right, bottom);
